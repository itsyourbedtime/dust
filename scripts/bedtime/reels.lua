...
local c_pos_x = 20
local c_pos_y = 58
local bind_vals = {19,49,79,108,0,0,0,0,0,0}
...
local function rec(tr, state)
  ...
  elseif state == false then -- stop rec
    ...
    reel.clip[tr] = 1
    counter:stop()
     -- unmute empty track after first recording
  end
  if (reel.name[tr] == "*-" and mutes[tr] == true) then
    mute(tr,false)
  end
  update_params_list()
end
...
function enc(n, d)
  ...
  elseif n == 2 then
    if not settings then
      speed = util.clamp(util.round((speed + d / 50), 0.001), -0.8, 0.8)
      if speed < 0 then
        reel.rev = 1
      elseif speed >= 0 then
        reel.rev = 0
